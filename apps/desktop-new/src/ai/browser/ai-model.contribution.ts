...
export class AIModelContribution implements PreferenceContribution, SettingContribution, ClientAppContribution {
  ...
  private async checkModelConfig() {
    // Check if a local model baseUrl is configured (e.g., Ollama at localhost:11434)
    // If baseUrl is set, skip API key checks since local LLMs don't need keys
    const baseUrl = this.preferenceService.getValid(ModelSettingId.baseUrl);
    if (baseUrl) {
      return;
    }
