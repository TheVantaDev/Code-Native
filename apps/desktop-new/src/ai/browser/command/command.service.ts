  ...
  public async getModelResp(input: string): Promise<IAIBackServiceResponse<ISumiModelResp>> {
    ...
    if (inputMatchedCommand) {
      ...
      return {
        data: {
          type: 'command',
          commandKey: id,
          answer: `Found suitable feature: ${labelLocalized?.localized || label}, you can use the (\`${id}\`) command`,
        },
      };
    }
    ...
    if (modelReply.errorCode || !modelReply.data) {
      ...
      if (!modelReply.errorMsg) {
        modelReply.errorMsg = 'AI model call failed';
      }
    ...
    if (data.id) {
      ...
      if (this.commandRegistryService.getCommand(data.id)) {
        sumiData = {
          type: 'command',
          commandKey: data.id,
          answer: `You can execute the (\`${data.id}\`) command: ${data.label}`,
        };
      }
      ...
      if ((this.preferenceService as any).has(data.id)) {
        sumiData = {
          type: 'setting',
          settingKey: data.id,
          answer: `You can configure via ${data.id}`,
        };
      }
    }
